...
import { ArrowDownRight, ArrowUpRight, CheckCircle, Clock, Trash2 } from "lucide-react";
...
export function ActiveAlertsTable({
  ...
  return (
    <div className="space-y-4">
      {/* 1. MOBILE TOUCH CARDS VIEW (Visible only on Mobile screens < md) */}
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {alert.direction} {alert.threshold_percent ? `(${alert.threshold_percent}%)` : ""}
                  </span>

                  <button
                    onClick={() => onDeleteAlert(alert.id)}
                    title="Delete Alert"
                    className="p-2 rounded-xl text-gray-400 hover:text-red-400 hover:bg-red-950/40 border border-transparent hover:border-red-800/40 transition-colors"
                  >
              ...
              <div className="grid grid-cols-3 gap-2 p-2.5 rounded-xl bg-background/60 border border-surface-border text-center font-mono">
                <div>
                  ...
                  <span className="text-xs font-bold text-gray-300">
                    ₹{alert.reference_price.toLocaleString("en-IN", { minimumFractionDigits: 1 })}
                  </span>
                </div>
                <div>
                  <span className="text-[9px] text-gray-400 block uppercase">Target</span>
                  <span className="text-xs font-extrabold text-white">
                    ₹{alert.target_price.toLocaleString("en-IN", { minimumFractionDigits: 1 })}
                  </span>
                </div>
                <div>
                  <span className="text-[9px] text-cyan-400 block uppercase font-sans">Live LTP</span>
                  <span className="text-xs font-extrabold text-cyan-300">
                    ₹{livePrice.toLocaleString("en-IN", { minimumFractionDigits: 1 })}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-1 text-[11px]">
                <span
                  className={`font-mono ${
                    Math.abs(distance) < 0.5 ? "text-amber-400 font-bold animate-pulse" : "text-gray-400"
                  }`}
                >
                  {distance > 0 ? "+" : ""}
                  {distance.toFixed(2)}% to target
                </span>
      ...
      {/* 2. DESKTOP/TABLET DATA TABLE VIEW (Visible on screens >= md) */}
